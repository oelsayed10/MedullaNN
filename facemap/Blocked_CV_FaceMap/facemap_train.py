...
def train_and_test_model(X, Y, KeypointsNetwork, out_list, sessions_list, Loss_fcn, extract_batches, create_dataloader, device, num_epochs, Animal, FOLD_NO):
    ...
    if Animal == 0: 
        ...
        test_loaders = None
        training_sessions = sessions_list

    elif Animal == 1: 
        train_loaders = Loaders[6:43]
        test_loaders = Loaders[0:6]
        training_sessions = sessions_list[6:43]

    elif Animal == 2:
        train_loaders = Loaders[0:6] + Loaders[14:43]
        test_loaders = Loaders[6:14]
        training_sessions = np.concatenate((sessions_list[0:6], sessions_list[14:43]))

    elif Animal == 3:
        train_loaders = Loaders[0:14] + Loaders[24:43]
        test_loaders = Loaders[14:24]
        training_sessions = np.concatenate((sessions_list[0:14], sessions_list[24:43]))

    elif Animal == 4: 
        train_loaders = Loaders[0:24] + Loaders[27:43]
        test_loaders = Loaders[24:27]
        training_sessions = np.concatenate((sessions_list[0:24], sessions_list[27:43]))

    elif Animal == 5: 
        train_loaders = Loaders[0:27] + Loaders[37:43]
        test_loaders = Loaders[27:37]
        training_sessions = np.concatenate((sessions_list[0:27], sessions_list[37:43]))

    elif Animal == 6: 
        train_loaders = Loaders[0:37]
        test_loaders = Loaders[37:43] 
        training_sessions = sessions_list[0:37]
    ...
    model = KeypointsNetwork(n_in = 55, n_kp = None, out_list = out_list, sessions_list = training_sessions)
    model.to(device)
    ...
    if test_loaders is None:
        model_save_path = f'Model_Trained_on_All_FaceMap.pth'
    else: 
        model_save_path = f'Model_tested_on_{Animal}_FOLD{FOLD_NO}_FaceMap.pth'
    ...
    if test_loaders is None:
        ...
        for test_loader in test_loaders:
            model.load_state_dict(torch.load(f'Model_tested_on_{Animal}_FOLD{FOLD_NO}_FaceMap.pth'))
            model.eval()
