...
def preprocess_data(X, Y, trial_len, session_id):
    ...
    nTrials = X.shape[0] // trial_len

    Y_pre = Y.reshape((Y.shape[0] // trial_len, trial_len, Y.shape[1]))
    ...
    for trial in range(nTrials): 
        trialY = Y_pre[trial, :, :].squeeze()
        # print('Shape of trialY: ', trialY.shape)

        A = (trialY - np.mean(Y, axis = 0)).T
        U, S, Vt = np.linalg.svd(A, full_matrices = False)
        V = Vt.T

        # Get Variance Explained 
        varExpTrial = [(S[i]**2) / np.sum(S**2) for i in range(len(S))]
        S = np.diag(S)

        trialY_pc = V @ S

        varExpTrials.append(varExpTrial)

        pcTrials.append(trialY_pc)

    Y_pc = np.concatenate(pcTrials, axis = 0)
    ...
    return X_reshaped, Y_reshaped, varExpTrials
